...
class Pshell(object):
    ...
    def plot(self, P):
        ...
        plt.legend(("Monte Carlo estimate (considered exact)", "Central Limit Theorem (CLT) estimate",
                    "Adjusted CLT estimate"), loc=4)
...
class FBplot(object):
    ...
    def plot(self):
        ...
        plt.fill_between(numpy.array(numpy.log10(n2)), self.arraylow[1:] / 2., self.arrayhigh[1:] / 2., color='red',
                         alpha=.5)
        ax = plt.gca()
...
def diffDensity(fig, F2, F3, Trange):
    ...
    cset = plt.contourf(X, Y, PXP, levels=numpy.array([-10.0, 0, .8]), colors=('r', 'g'),
