...
if __name__ == '__main__':
    RTG50 = TruncatedGaussian(cv=.5)
    RTG10 = TruncatedGaussian(cv=.1)
    RTG4 = TruncatedGaussian(cv=.04)
    RIG50 = InverseGaussian(cv=.5)
    RIG10 = InverseGaussian(cv=.1)
    RIG4 = InverseGaussian(cv=.04)
    TG50 = RTG50.flatten(name='Truncated Gaussian, C.V. = 50%')
    TG10 = RTG10.flatten(name='Truncated Gaussian, C.V. = 10%')
    TG4 = RTG4.flatten(name='Truncated Gaussian, C.V. = 4%')
    IG50 = RIG50.flatten(name='Inverse Gaussian, C.V. = 50%')
    IG10 = RIG10.flatten(name='Inverse Gaussian, C.V. = 10%')
    IG4 = RIG4.flatten(name='Inverse Gaussian, C.V. = 4%')
    IG50.sim(1000)
    TG50.sim(1000)
    IG10.sim(1000)
    TG10.sim(1000)
    IG4.sim(1000)
    TG4.sim(1000)
    IG50.compare_pdfs(TG50, a=0.001, b=6, xlab='Intertick Interval Normalized to Multiples of Period')
    IG10.compare_pdfs(TG10, a=0.001, b=6, xlab='Intertick Interval Normalized to Multiples of Period')