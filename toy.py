...
class ToySimulator(object):
    def __init__(self, seed=None, experiment=None):
        self.initRNG(seed)
    ...
    def initRNG(self,seed=None): # Maybe overloaded if using a different RNG, eg rpy2
        self.R = RNG(seed)   # RNG is a class
    ...
    def restart(self):   # Clears data and resets RNG with same seed
        self.R.reset()   # Resets RNG with same seed as used before
...
class ToyLiker(object):
    ...
    def restart(self):
        self.likes = []
        self.changedSinceLastLike = False
